...
class Note extends FNFSprite
{
	...
	**/
	public function mustPres()
	{
		return (mustPress ? PlayState.dadOpponent.characterData.skin : PlayState.boyfriend.characterData.skin);
	}

	public static function returnDefaultNote(assetModifier, strumTime, noteData, noteType, noteAlt, ?isSustainNote:Bool = false, ?prevNote:Note = null, skin:String):Note
	{
		var newNote:Note = new Note(strumTime, noteData, noteAlt, prevNote, isSustainNote);

		//need to see if it is bf or dad note.
		//this.skin = skin;

		//this not might work, but I have no idea how notes generate.
		...
		switch (assetModifier)
		{
			case 'pixel': // pixel arrows default
				if (isSustainNote)
				{
					newNote.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('arrowEnds', assetModifier, skin,
				...
				{
					newNote.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('arrows-pixels', assetModifier, skin,
						'noteskins/notes')),
						true, 17, 17);
			...
			default: // base game arrows for no reason whatsoever
				newNote.frames = Paths.getSparrowAtlas(ForeverTools.returnSkinAsset('NOTE_assets', assetModifier, skin,
					'noteskins/notes'));
	...
	public static function returnQuantNote(assetModifier, strumTime, noteData, noteType, noteAlt, ?isSustainNote:Bool = false, ?prevNote:Note = null, skin:String):Note
	{
		var newNote:Note = new Note(strumTime, noteData, noteAlt, prevNote, isSustainNote);
		...
		switch (assetModifier)
		{
			default:
				...
				if (!isSustainNote)
				{
					...
					newNote.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('NOTE_quants', assetModifier, skin,
				...
				{
					// quant holds
					newNote.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('HOLD_quants', assetModifier, skin,
						'noteskins/notes', 'quant')),
