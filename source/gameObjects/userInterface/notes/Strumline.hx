...
class UIStaticArrow extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?babyArrowType:Int = 0, skin:String = "default")
	{
...
class Strumline extends FlxTypedGroup<FlxBasic>
{
			...
			?noteSplashes:Bool = false, ?keyAmount:Int = 4, ?downscroll:Bool = false, ?parent:Strumline)
	{
		...
		this.skin = character.characterData.skin;

		for (i in 0...keyAmount)
		{
			var staticArrow:UIStaticArrow = ForeverAssets.generateUIArrows(-25 + x, 25 + (downscroll ? FlxG.height - 200 : 0), i, PlayState.assetModifier, skin);
			staticArrow.ID = i;
			...
			if (noteSplashes) {
				noteSplash = ForeverAssets.generateNoteSplashes('noteSplashes', PlayState.assetModifier, PlayState.changeableSkin, 'UI', i);
		...
		{ trace("knew this was going to happen, an error occured generating strums. error message is: " + e.message + " | pls send back -Discussions"); }
	}
