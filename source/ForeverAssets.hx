...
class ForeverAssets
{
	...
	public static function generateNoteSplashes(asset:String, assetModifier:String = 'base', changeableSkin:String = 'default', baseLibrary:String, noteData:Int):NoteSplash
	{
		...
		switch (assetModifier)
		{
			...
			default:
				...
				if (PlayState.SONG.player1 == 'bf-kadyplayable') //the only way this could be a problem is if someone else is player2 and kady is player 1 and if notesplashes are on the cpu side too
					tempSplash.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('noteSplashes-kady', assetModifier, changeableSkin, baseLibrary)), true, 210, 210);
				else
	...
	public static function generateUIArrows(x:Float, y:Float, ?staticArrowType:Int = 0, assetModifier:String, skin:String):UIStaticArrow
	{
		var newStaticArrow:UIStaticArrow = new UIStaticArrow(x, y, staticArrowType, skin);
		trace("looking for: " + ForeverTools.returnSkinAsset("<Note_File_Name>", assetModifier, skin, 'noteskins/notes'));
		...
		switch (assetModifier)
		{
			case 'pixel':
				...
				newStaticArrow.loadGraphic(Paths.image(ForeverTools.returnSkinAsset('$framesArgument', assetModifier, skin,
	...
	/**
		Notes!
	**/
	public static function generateArrow(assetModifier, strumTime, noteData, noteType, noteAlt, ?isSustainNote:Bool = false, ?prevNote:Note = null, skin:String /*this will make an error, saying that it is missing "skin"*/):Note
	{
		var newNote:Note;
		var changeableSkin:String = skin;
		// gonna improve the system eventually
		if (changeableSkin.startsWith('quant'))
			newNote = Note.returnQuantNote(assetModifier, strumTime, noteData, noteType, noteAlt, isSustainNote, prevNote, skin);
		else
			newNote = Note.returnDefaultNote(assetModifier, strumTime, noteData, noteType, noteAlt, isSustainNote, prevNote, skin);
