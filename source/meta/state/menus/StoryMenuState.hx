...
class StoryMenuState extends MusicBeatState
{
	...
	function selectWeek()
	{
		if (weekUnlocked[curWeek])
		{
			if (!stopspamming)
			{
				...
				if (curWeek == 0)
				{
					grpWeekCharacters.members[1].x -= 25;
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		switch (curDifficulty) //lower numbers go right, higher numbers go left??
		{
			case 0:
		...
		//trace(curDifficulty + " " + CoolUtil.difficultyFromNumber(curDifficulty));
