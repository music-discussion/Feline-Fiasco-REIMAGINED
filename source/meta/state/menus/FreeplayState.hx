...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		**/
		...
		bd.color = FlxColor.fromHSB(0, 0, 0.5, 1); //dimming the backdrop
		add(bd);
	...
	override function update(elapsed:Float)
	{
		...
		if (songs[curSelected].songName.toLowerCase() == 'secret') //not doing this will show the difficulty as 'SIMPLE'
		{
			diffText.text = '< AWESOME! >';
			diffText.color = FlxColor.RED;
		}	
		
		if (FlxG.keys.justPressed.G)
			trace("\nACHIEVEMENT INFO! "
			+"\nWeek Completed? " + FlxG.save.data.weekCompleted
			+ "\nGanghan Unlock: " + FlxG.save.data.gangUnlock
			+ "\nBalam Unlock: " + FlxG.save.data.balamUnlock
		...
		if (accepted)
		{
			...
			if (songs[curSelected].songName.toLowerCase() == 'secret')
