			...
			default:
				...
				for (section in noteData) {
					...
					for (songNotes in section.sectionNotes)
					{
						...
						//okay since they make everything so complicated
						//we are going to do skin by mustHit for testing purposes.
						var skin:String = "default";
						//trace("note skin before: " + skin); //do not uncomment these if you dont want 30 second loading times!!
						if (!gottaHitNote) 
							{skin = PlayState.dadOpponent.characterData.skin;} //trace("dad note skin after: " + skin);}
						else 
							{skin = PlayState.boyfriend.characterData.skin;} //trace("bf note skin after: " + skin); }
						// create the new note
						var swagNote:Note = ForeverAssets.generateArrow(PlayState.assetModifier, daStrumTime, daNoteData, 0, daNoteAlt, skin);
						// set note speed
						...
						for (susNote in 0...Math.floor(susLength))
						{
							...
							var sustainNote:Note = ForeverAssets.generateArrow(PlayState.assetModifier,
								daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, 0, daNoteAlt, true, oldNote, skin);
